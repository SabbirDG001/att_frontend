...
document.getElementById('submit').addEventListener('click', async () => {
    ...
    try {
        ...
        const attendanceCount = parseInt(document.getElementById("atdcnt").value);
        ...
        document.querySelectorAll('input[type="checkbox"]').forEach(checkbox => {
            ...
            attendanceData.students.push({
                ...
                status: checkbox.checked ? 1 * attendanceCount : 0
            });
        });
...
document.getElementById('see_all').addEventListener('click', async () => {
    ...
    try {
        ...
        // Calculate total attendance count available across all dates
        let totalAttendancePoints = 0;
        attendanceData.forEach(record => {
            // Add this record's attendance points to the total
            const recordAttdnc = parseInt(record.attdnc) || 1;
            totalAttendancePoints += recordAttdnc;
            console.log(`Date: ${record.date}, Attendance Value: ${recordAttdnc}, Running Total: ${totalAttendancePoints}`);
        });
        ...
        Object.entries(studentAttendance).forEach(([studentId, data]) => {
            ...
            attendanceData.forEach(record => {
                ...
                const status = student ? student.status : 'N/A';
                row.innerHTML += `<td>${status}</td>`;
            });

            // Calculate and add percentage
            const attendancePercentage = (data.attendanceCount / totalAttendancePoints * 100).toFixed(2);
...
const API_URL = 'https://att-backend-ipnsm8oht-sabbirdg001s-projects.vercel.app/api';